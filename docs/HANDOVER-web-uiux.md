> ⚠️ **現状 + 未決 (2026-09-23 時点)**: この文書は 2026-09-06 時点のブランチ状態のスナップショットで、
> その後のリファクタで一部の部品・パス・行番号が変わっている可能性がある (X-03 で「存在しない部品・
> パスへの言及」として指摘された)。個別の対応状況を今回の棚卸しでは全件突き合わせできていない。
> 実装の正は `web/src/**` そのものと [`ARCHITECTURE-web.md`](ARCHITECTURE-web.md) (最新の状態を追記済み)。
> この文書は「当時の引き継ぎ経緯の記録」として読み、個々のファイル名/行番号は鵜呑みにしないこと。

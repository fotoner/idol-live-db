...
final class BulkImageImporter {
    ...
    /// 進み具合・結果の表示 (文言は画面の言語で描くため、解決前の値で持つ)。
    var statusMessage: DisplayText = .verbatim("")
    var importedCount = 0
    var failedCount = 0
    ...
    struct Failure: Identifiable, Equatable {
        var id: String { key }
        let key: String
        /// 失敗の理由。アプリの文言 (.key) か、OS のエラー文・HTTP の状態 (.verbatim)。
        let reason: DisplayText
    }
    ...
    func importBrandImagesFromURL(_ urlString: String, database: AppDatabase) async {
        await runImport(
            urlString: urlString,
            notFound: L10n.Settings.imageImportFailureBrandNotFound,
            run: { _ in
    ...
    func importUnitImagesFromURL(_ urlString: String, database: AppDatabase) async {
        await runImport(
            urlString: urlString,
            notFound: L10n.Settings.imageImportFailureUnitNotFound,
            run: { _ in
    ...
    func importFromURL(_ urlString: String, database: AppDatabase) async {
        await runImport(
            urlString: urlString,
            notFound: L10n.Settings.imageImportFailureIdolNotFound,
            run: { _ in
    ...
    func clearAllImages() async {
        do {
            ...
            statusMessage = .key(L10n.Settings.imageImportStatusCleared)
            failures = []
            importedCount = 0
            failedCount = 0
        } catch {
            statusMessage = .key(L10n.Settings.imageImportStatusClearFailed(detail: error.localizedDescription))
        }
    }
    ...
    private func runImport(
        ...
        failedCount = 0
        failures = []
        statusMessage = .key(L10n.Settings.imageImportStatusFetching)

        do {
            let (data, _) = try await URLSession.shared.data(from: url)
            guard let mapping = try? JSONDecoder().decode([String: String].self, from: data) else {
                statusMessage = .key(L10n.Settings.imageImportStatusInvalidJson)
                isImporting = false
                return
            }
            ...
            for (key, imageURLString) in mapping {
                ...
                guard let id = nameToId[key] ?? nameToId[normalizedKey] else {
                    failedCount += 1
                    failures.append(Failure(key: key, reason: .key(notFound)))
                    continue
                }

                guard let imageURL = URL(string: trimmed) else {
                    failedCount += 1
                    failures.append(Failure(key: key, reason: .key(L10n.Settings.imageImportFailureInvalidUrl)))
                    continue
                }
                ...
                do {
                    ...
                    if let http = response as? HTTPURLResponse, !(200..<300).contains(http.statusCode) {
                        failedCount += 1
                        failures.append(Failure(key: key, reason: .verbatim("HTTP \(http.statusCode)")))
                        continue
                    }
                    guard let image = UIImage(data: imageData) else {
                        failedCount += 1
                        failures.append(Failure(key: key, reason: .key(L10n.Settings.imageImportFailureDecodeFailed)))
                        continue
                    }
                    try await save(id, image)
                    importedCount += 1
                    statusMessage = .key(L10n.Settings.imageImportStatusProgress(imported: importedCount, total: total))
                } catch {
                    failedCount += 1
                    failures.append(Failure(key: key, reason: .verbatim(error.localizedDescription)))
                }

                try? await Task.sleep(for: .milliseconds(100))
            }

            statusMessage = .key(L10n.Settings.imageImportStatusDone(succeeded: importedCount, failed: failedCount))
        } catch {
            statusMessage = .key(L10n.Settings.imageImportStatusError(detail: error.localizedDescription))
        }

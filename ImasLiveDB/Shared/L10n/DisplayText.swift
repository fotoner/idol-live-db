...
/// DS コンポーネントはこれだけを受ける — リテラルが LocalizedStringResource に化けて
/// 翻訳引き・ロケール書式 (桁区切り) の経路へ黙って入るのを型で塞ぐため。
///
/// - 文言: `.key(L10n.X.y)`
/// - データ: `.verbatim(name)` (曲名・アイドル名・サーバの文言・ユーザー入力・書式済みの数値)
/// - コア由来: `.core(label)` (imas-core が作った完成文字列。コア段階で置き換える箇所の目印)
enum DisplayText: Sendable, Equatable {
    case key(LocalizedStringResource)
    case verbatim(String)
    /// 今は verbatim と同じに出す。`rg 'DisplayText\.core|\.core\('` がコア段階の置換リストになる。
    case core(String)

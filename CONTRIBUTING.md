...
韓国語・英語などへの対応は大歓迎です。大きく進める前に、進め方を Issue で相談させてください。

- **いまの状態**: iOS / Android の UI 文言は `i18n/catalog` に置き、`tools/i18n` が iOS の String Catalog と Android の `strings_<ns>.xml` を生成する土台があります (書き方は [`i18n/README.md`](i18n/README.md)、訳し方は [`i18n/TRANSLATION.md`](i18n/TRANSLATION.md))。ただし画面の文字列はまだほとんどコードに日本語で直接書かれていて、画面ごとにカタログへ移していきます。Web は対象外です。
- **カタログに移した画面の文言を足す・変えるとき**: `python3 tools/i18n/i18n.py add` などでカタログを直し、生成物も同じ commit に入れます (生成物は手で直しません)。

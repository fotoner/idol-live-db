...
## 3. 歌詞サイトとの照合・取り込み・版違いの流用

取り込み元の歌詞サイトに依存するツール (照合と取り込み) と、それを使う
`copy_variants.py` (版違いに親の歌詞を流用する) は、公開リポジトリには置かない。
手元の `lyrics_local/tools/` にあり、使い方も同じ場所の README に書いてある。
共有の `link_verify.py` / `lyrics_json.py` はここに残っていて、あちらから読む。

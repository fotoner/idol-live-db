...
class AndroidResourceTest(unittest.TestCase):
    ...
    def test_locale_config_only_in_debug_while_release_has_one_language(self):
        # release の言語が ja だけのうちは、Release の「アプリの言語」の一覧に出さない (main に置かない)
        self.assertNotIn(RES + "/main/xml/locale_config.xml", self.files)
        self.assertEqual(self.files[RES + "/debug/xml/locale_config.xml"].count("<locale "), 3)
...
class ChannelPlacementTest(unittest.TestCase):
    ...
    def test_release_goes_to_main(self):
        ...
        self.assertFalse(any(p.startswith(RES + "/debug/values") for p in files))
        self.assertEqual(files[RES + "/main/xml/locale_config.xml"].count("<locale "), 2)

...
KINDS = ("ui", "system", "content")
# 低い順。planned はどのビルドにも入らない (カタログ・用語集に値を持てるだけ)
...
NS_FIELDS = ("namespace", "kind", "ios_bundles", "platforms", "strings")
ENTRY_FIELDS = ("args", "note", "platforms", "max_len", "keep_whitespace", "verbatim_ok", "ios")
IOS_ENTRY_FIELDS = ("infoplist", "intent_metadata")
CONFIG_FIELDS = ("source_language", "languages")
GLOSSARY_FIELDS = ("note", "status", "style", "terms")
GLOSSARY_TERM_FIELDS = ("ja", "note", "keep")  # これと言語コード

# テーブル名として使えない
# Localizable / InfoPlist は Xcode の予約表。L10n〜Bundle は生成 Swift が使う識別子で、
# 同名の enum が extension L10n の中にできると生成物全体が型を見失う。Type〜Protocol は
# Swift が型の名前に使わせない。CatalogKeys / CatalogSample は Kotlin のテスト用の生成物
# (L10nCatalogKeys / L10nCatalogSample) と名前がぶつかる
RESERVED_TABLES = ("Localizable", "InfoPlist",
                   "L10n", "String", "Int", "LocalizedStringResource", "Foundation", "Bundle",
                   "Type", "Self", "Any", "Protocol",
                   "CatalogKeys", "CatalogSample")
...
def build_config(path, raw, problems):
    ...
    return Config(source_language=src, languages=ordered)
...
def _build_entry(ns, path, key, raw, config, ns_platforms, problems):
    ...
    for k, v in raw.items():
        ...
        if k in config.languages:
            if isinstance(v, str):
                ...
                empty = sorted(c for c, t in v.items() if t == "")
                if empty:
                    problems.append(error(where, key, "%s の %s が空文字列 (その範疇が要らないなら書かない)" % (
                        k, ", ".join(empty))))
                    ok = False
                    continue
                values[k] = dict(v)
    ...
    if "platforms" in raw:
        ...
        if not _is_str_list(p) or not p:
            ...
            if bad:
                problems.append(error(where, key, "platforms に %s は書けない (ios / android だけ)" % ", ".join(bad)))
                ok = False
...
def build_namespace(path, stem, raw, config, problems):
    ...
    if not _is_str_list(platforms) or not platforms or any(p not in PLATFORMS for p in platforms):
        problems.append(error(path, None, "platforms は [\"ios\", \"android\"] の空でない部分集合にする"))
...
def build(raw_repo, problems):
    ...
    return Catalog(config=config, namespaces=namespaces, glossary=glossary)

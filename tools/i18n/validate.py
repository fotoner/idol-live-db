...
名前・値・プラットフォーム・品質・出荷ゲート・project.yml を見る。生成したリソースの
不変式は emit_android.invariant_problems。

機械で白黒が付くもの (プレースホルダの組・max_len・出荷ゲート) はエラー、判断の要るもの
(用語集・仮名の混じり・複数形の範疇の不足) は警告にする。

外部の入力 (project.yml の本文) は呼び出し側が渡す。ここではファイルを読まない。
"""
...
# コードから写し間違えたプラットフォームの書式
_PLATFORM_SYNTAX = [
    # %,d (Android の桁区切り) / %.1f / %i なども拾う。"100%"・"50%オフ"・"50% off"・"50%OFF" のような
    # 素の % は通す (空白のフラグは取らず、変換文字のすぐ後ろに英字が続くものは書式とみなさない)
    (re.compile(r"%(?:\d+\$)?[-+0#,]*\d*(?:\.\d+)?(?:ll|l|h|q)?[@dDiuUxXoOfFeEgGcCsSp](?![A-Za-z])"),
...
def validate(catalog, project_yml=None, external=True):
    """問題の一覧を返す。external=False なら project.yml との照合を飛ばす。"""
    out = []
    _names(catalog, out)
    ...
    _language_tag(catalog, out)
    _text_sample(catalog, out)
    _gate(catalog, out)
    if external and ios_outputs(catalog):
        _project_yml(catalog, project_yml, out)
    return out


def ios_outputs(catalog):
    """iOS に出す項目 (表・Info.plist) が 1 つでもあるか。project.yml の照合はそのときだけする。"""
    return any(e.on("ios") for ns in catalog.namespaces for e in ns.entries)
...
def _names(catalog, out):
    ...
    for ns in catalog.namespaces:
        ...
        if t in model.RESERVED_TABLES:
            out.append(error(ns.path, None, "名前空間 %s の表名 %s は予約名なので使えない (Xcode・Swift・生成物が使う名前)" % (ns.name, t)))
        if t in tables:
...
def _note(ns, e, out):
    # note は生成物のコメント (Swift の ///・XML の <!-- -->) に入る。CR などはコメントを割って生成物を壊す
    if e.note is not None and _BAD_CONTROL.search(e.note):
        out.append(error(ns.path, e.key, "note に改行 (\\n) 以外の制御文字がある (タブ・CR など)"))
...
def _language_tag(catalog, out):
    """i18n.language_tag は、ビルドに入る言語ごとに、その言語のコードそのものを値に持つ。

    アプリ (DisplayLocale) はこの値で「実際に画面に出ている言語」を知る。値が抜けた言語では
    基準言語の値に落ちて、訳は出ているのに言語だけが基準言語だと思い込む。
    """
    e = catalog.find(model.LANGUAGE_TAG_KEY)
    if e is None:
        return
    ns = catalog.namespace(e.ns)
    if e.args:
        out.append(error(ns.path, e.key, "%s は引数を持てない" % model.LANGUAGE_TAG_KEY))
    for lang in catalog.config.languages:
        built = catalog.config.channel(lang) != "planned"
        if not e.has(lang):
            if built:
                out.append(error(ns.path, e.key, "%s に %s の値が無い (値は言語コードそのもの: \"%s\")" % (
                    model.LANGUAGE_TAG_KEY, lang, lang)))
            continue
        if e.values[lang] != lang:
            out.append(error(ns.path, e.key, "%s の %s の値は言語コードそのもの (\"%s\") にする (今 %r)" % (
                model.LANGUAGE_TAG_KEY, lang, lang, e.values[lang])))
...
def _gate(catalog, out):
    """beta / release の言語は、ui / system の名前空間に訳の欠落を残さない (dev と planned は止めない)。"""
    config = catalog.config
    for lang, channel in config.languages.items():
        ...
        for ns in catalog.namespaces:
            if ns.kind not in ("ui", "system"):
                continue
            n = sum(1 for e in ns.entries if not e.has(lang))
            if n:
                missing[ns.name] = n
        if missing:
            out.append(error(model.CONFIG_PATH, None, "%s が %s なのに訳の欠落が %d 件 (%s)" % (
                lang, channel, sum(missing.values()), ", ".join(sorted(missing))),
                "python3 tools/i18n/i18n.py stats で一覧"))

...
- ImasLiveDB-Android/app/i18n/<overlay>/values[-xx]/strings_<ns>.xml
  ...
  channel=planned の言語はどこにも置かない (locale_config・テストの期待値にも出さない)。
- xml/locale_config.xml (Android 13 以降の「アプリの言語」に出す言語)。debug には常に置く
  (src/debug/AndroidManifest.xml が参照する)。main には release の言語が 2 つ以上になったときだけ置く
  (1 つだけなら OS の一覧に出す意味が無い。そのときは src/main/AndroidManifest.xml へ参照を移す)。
...
def locale_config_overlays(config):
    """locale_config.xml を置く overlay。debug は常に (manifest が参照する)、main は release の言語が 2 つ以上のとき。"""
    overlays = ["debug"]
    if len(overlay_languages(config, "main")) >= 2:
        overlays.insert(0, "main")
    return overlays
...
def emit(catalog):
    ...
    for overlay in locale_config_overlays(config):

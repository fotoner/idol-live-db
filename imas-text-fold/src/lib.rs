...
//!
//! **例外: 歌詞検索はこの規則を使っていない** (Q-06 で「変えない・記録だけする」と決めた)。
//! 歌詞本文はサーバ (D1) にしか無く、Worker の `normalizeForSearch`
//! (`imas-live-api/src/routes/lyrics.ts`) が別に畳む。向きが逆で、ここはカタカナ→ひらがな、
//! あちらはひらがな→カタカナ。あちらは全角英数→半角も畳み、濁点の合成はしない。
//! 転置索引 (body_norm と gram 表) がその向きで組まれているので、揃えるなら索引の
//! 全再構築とセットになる。検索語をこちらで畳んでからサーバへ渡さないこと
//! (あちらが畳み直すので当たりは変わらないが、二重に畳む前提が生まれる)。
//!

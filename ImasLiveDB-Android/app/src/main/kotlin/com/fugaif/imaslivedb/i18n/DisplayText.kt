 ...
 *
 * DS コンポーネントはこの型だけを受ける。文字列リテラルからは作れないので、データが訳語として
 * 引かれる事故と、文言が翻訳から黙って漏れる事故の両方を型で止める。
 */
@Immutable
sealed interface DisplayText {
     ...
     * 目印として分けておき、コア段階でキーを返すようになったら置き換える (置換リスト)。
     */
...
/** コア由来の文字列をそのまま出すときの目印 (iOS の `Text(core:)` に当たる)。値は変えない。 */
fun coreText(value: String): String = value

...
object L10nSearch {
    /** {tab}に {count} — 他のタブの一致件数へ飛ぶチップ。tab はタブ名 (imas-core の AppDestination.label をそのまま渡す。今は日本語のまま)、count は件数。件数は今の表示どおり桁区切りしない (int) — 引数: tab (core), count (int) */
    fun crossTabChip(tab: String, count: Int): DisplayText = DisplayText.Res(R.string.search_cross_tab_chip, listOf(tab, count))
